...
class Assert {
    ...
    private Assert() {
        // Checkstyle rules enforce non-public constructor in utility classes
    }

    static void assertStatusCode(final int statusCode, final HttpResponse httpResponse) {
        assertEquals("Expected different status code", statusCode, httpResponse.getStatusLine().getStatusCode());
    }

    static void assertContentType(final String contentType, final HttpResponse httpResponse) {
